#include "stdafx.h"
#include "..\Headers\Scene_Camera.h"

_USING(MapTool)

CScene_Camera::CScene_Camera(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CScene(pGraphic_Device)
{
}

HRESULT CScene_Camera::Ready_Scene()
{
	// For.Light Setting
...
_int CScene_Camera::Update_Scene(const _float & fTimeDelta)
{
	return _int(CScene::Update_Scene(fTimeDelta));
}

_int CScene_Camera::LateUpdate_Scene(const _float & fTimeDelta)
{
	return _int(CScene::LateUpdate_Scene(fTimeDelta));
}

HRESULT CScene_Camera::Render_Scene()
{
	return CScene::Render_Scene();
}

HRESULT CScene_Camera::Ready_LightInfo()
{
	D3DLIGHT9				LightInfo;
...
HRESULT CScene_Camera::Ready_Component_Prototype()
{
...
HRESULT CScene_Camera::Ready_GameObject_Prototype()
{
...
HRESULT CScene_Camera::Ready_Layer_Camera(const _tchar * pLayerTag)
{
	//// For.Camera
...
CScene_Camera * CScene_Camera::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CScene_Camera* pInstance = new CScene_Camera(pGraphic_Device);

	if (FAILED(pInstance->Ready_Scene()))
	{
		_MSGBOX("CScene_Camera Created Failed");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CScene_Camera::Free()
{
	CScene::Free();
}
