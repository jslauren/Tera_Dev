...
_int CSkyBox::LateUpdate_GameObject(const _float & fTimeDelta)
{
	...
	CTransform*	pCameraTransform = (CTransform*)pObject_Manager->Get_Component(SCENE_PROTO, L"Layer_Camera", L"Com_Transform");
...
HRESULT CSkyBox::Add_Component()
{
	...
	// For.Com_Buffer
	if (FAILED(CGameObject::Add_Component(SCENE_PROTO, L"Component_Buffer_CubeBox", L"Com_Buffer", (CComponent**)&m_pBufferCom)))
		return E_FAIL;
	...
	// For.Com_Texture
	if (FAILED(CGameObject::Add_Component(SCENE_PROTO, L"Component_Texture_SkyBox", L"Com_Texture", (CComponent**)&m_pTextureCom)))
		return E_FAIL;
