...
HRESULT CQuadTree::Culling(CFrustum * pFrustum, CTransform* pTransform, INDEX16* pIndices, _uint& iNumPolygon)
{
	// 가장 작은 사각형일때(자식없다)
	...
	if (true == LevelofDetail())
	{
			...
			true == isDraw[3])
		{
			...
			if (true == isIn[0] ||
				...
				true == isIn[2])
			{
				pIndices[iNumPolygon]._1 = m_iCornerIdx[CORNER_LT];
				pIndices[iNumPolygon]._2 = m_iCornerIdx[CORNER_RT];
				pIndices[iNumPolygon]._3 = m_iCornerIdx[CORNER_RB];
				++iNumPolygon;
			}
				...
				true == isIn[3])
			{
				...
				pIndices[iNumPolygon]._3 = m_iCornerIdx[CORNER_LB];
				++iNumPolygon;
			}
			return NOERROR;
		}
			...
			true == isIn[3])
		{
			if (false == isDraw[NEIGHBOR_LEFT])
			{
				...
				pIndices[iNumPolygon]._3 = iLC;
				++iNumPolygon;

				pIndices[iNumPolygon]._1 = iLC;
				pIndices[iNumPolygon]._2 = m_iCenterIdx;
				pIndices[iNumPolygon]._3 = m_iCornerIdx[CORNER_LB];
				++iNumPolygon;
			}
			...
			{
				pIndices[iNumPolygon]._1 = m_iCornerIdx[CORNER_LB];
				pIndices[iNumPolygon]._2 = m_iCenterIdx;
				pIndices[iNumPolygon]._3 = iBC;
				++iNumPolygon;

				pIndices[iNumPolygon]._1 = iBC;
				pIndices[iNumPolygon]._2 = m_iCenterIdx;
				pIndices[iNumPolygon]._3 = m_iCornerIdx[CORNER_RB];
				++iNumPolygon;
			}
			...
			true == isIn[2])
		{
			...
			{
				pIndices[iNumPolygon]._1 = m_iCornerIdx[CORNER_LT];
				pIndices[iNumPolygon]._2 = iTC;
				pIndices[iNumPolygon]._3 = m_iCenterIdx;
				++iNumPolygon;

				pIndices[iNumPolygon]._1 = m_iCenterIdx;
				pIndices[iNumPolygon]._2 = iTC;
				pIndices[iNumPolygon]._3 = m_iCornerIdx[CORNER_RT];
				++iNumPolygon;
			}
			else
			{
...
CQuadTree * CQuadTree::Create(LPDIRECT3DDEVICE9 pGraphic_Device, const _vec3* pPositions, const _uint& iNumVerticesX, const _uint& iNumVerticesZ)
{
	...
	{
		_MSGBOX("CQuadTree Created Failed");
