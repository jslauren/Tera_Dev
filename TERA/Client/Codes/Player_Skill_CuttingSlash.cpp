...
CPlayerState * CPlayer_Skill_CuttingSlash::Input_Keyboard(CPlayer & Player, const float & fTimeDelta, BYTE KeyID, void * pArg)
{
	if (Player.Get_Mesh_Bone()->Get_NowPlayAniIndex() == CPlayer::PLAYER_ANI::CuttingSlash)
	{
		...
		if (Player.Get_Mesh_Bone()->IsAnimationEnded(0.08f))
			SoundPlay(Player, 0);
		...
		if (Player.Get_Mesh_Bone()->IsAnimationEnded(0.9f))
		{
			...
			{
				AttackEventFree(&Player, CPlayer::PLAYER_ANI::CuttingSlash);
				m_iAniState = 2;
...
void CPlayer_Skill_CuttingSlash::SoundPlay(CPlayer & Player, _uint iSountNum)
{
	if (Player.Get_SoundCheckInfo() == true)
	{
		...
		if(iSountNum == 0)
			CSoundManager::GetInstance()->Play_SoundChannel("Slayer_CuttingSlash_Casting_00.ogg", CSoundManager::Channel_ID::CH_SKILL, false);
		else if(iSountNum == 1)
			CSoundManager::GetInstance()->Play_SoundChannel("Slayer_CuttingSlash_Casting_01.ogg", CSoundManager::Channel_ID::CH_SKILL, false);
