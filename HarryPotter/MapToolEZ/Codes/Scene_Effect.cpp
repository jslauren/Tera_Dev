#include "stdafx.h"
#include "..\Headers\Scene_Effect.h"

_USING(MapTool)

CScene_Effect::CScene_Effect(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CScene(pGraphic_Device)
{
}

HRESULT CScene_Effect::Ready_Scene()
{
	// For.Light Setting
...
_int CScene_Effect::Update_Scene(const _float & fTimeDelta)
{
	return _int(CScene::Update_Scene(fTimeDelta));
}

_int CScene_Effect::LateUpdate_Scene(const _float & fTimeDelta)
{
	return _int(CScene::LateUpdate_Scene(fTimeDelta));
}

HRESULT CScene_Effect::Render_Scene()
{
	return CScene::Render_Scene();
}

HRESULT CScene_Effect::Ready_LightInfo()
{
	D3DLIGHT9				LightInfo;
...
HRESULT CScene_Effect::Ready_Component_Prototype()
{
...
HRESULT CScene_Effect::Ready_GameObject_Prototype()
{
...
HRESULT CScene_Effect::Ready_Layer_Camera(const _tchar * pLayerTag)
{
	//// For.Camera
...
CScene_Effect * CScene_Effect::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CScene_Effect* pInstance = new CScene_Effect(pGraphic_Device);

	if (FAILED(pInstance->Ready_Scene()))
	{
		_MSGBOX("CScene_Effect Created Failed");
