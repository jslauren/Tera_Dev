...
public:
	_byte	GetDIKeyState(_ubyte byKey) {
		return m_KeyState[byKey]; }

	_byte	GetDIMouseState(MOUSEBUTTON eMouseBT) {
		return m_MouseState.rgbButtons[eMouseBT];
	}

	_byte	GetDIMouseMove(MOUSEMOVE eMouseMove) {
		return *((_long*)&m_MouseState + eMouseMove);
	}
public:
	HRESULT	Ready_Input_Device(HINSTANCE hInst, HWND hWnd);
	HRESULT	Investigate_Input_State();
private:
